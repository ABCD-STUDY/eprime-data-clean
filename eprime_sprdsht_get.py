...
def program_description():
    ...
    print('                     Octavio Ruiz.  2017jun05-nov01, 2018feb19-jun25, 2019apr05-jul23, nov13-dec04, 2020jan13-feb14')
    print()
    print('Usage:')
    ...
    print()
    print('  exper            Experiment found inside the file (short code)')
...
# ---------------------------------------------------------------------------------------------------------------
def extract_ini_time( data, Info ):
    ...
    # return delay, exp_t0, datime_ok, ok, msg
    return Info, ok, msg
# ---------------------------------------------------------------------------------------------------------------
    ...
    if exper in ['MID','SST']:
        ...
        if col in data.columns:
            ...
            for signal in signal_list:
                # Locate starting time of this run: value in CueFix.StartTime, in the row that follows the last occurrence of the specified signal
...
def EPrime_Info_and_Data_get( fname ):
    ...
    if Info['ok']:
        ...
        if  Info['exper'] == 'nBack' and ('Rec' in Info['exp_in_file'] or 'REC' in Info['exp_in_file']):
            ...
            Info, ok, msg  =  extract_ini_time( data, Info )
...
def List_and_Pick_Files( fname, optn, fname_out, subj, ref_time_arg, task_arg, optn2, header_show, quiet=False ):
    ...
    for j, fname_full in enumerate( f_list ):
        ...
        if EPrime_Info['fname']:
            ...
            if EPrime_Info['exper'] == 'nBack'  and  ('Rec' in EPrime_Info['exp_in_file'] or 'REC' in EPrime_Info['exp_in_file'])  and  EPrime_Info['datime_ok']  and  EPrime_Info['nruns'] == 0:
                if Verbose:
    ...
    if optn == 'PickFile':
        ...
        if 'Info' in optn2:
            ...
            if EPrime_Info['exper'] == task_arg:
                if EPrime_Info['exper'] == 'nBack' and ('Rec' in EPrime_Info['exp_in_file'] or 'REC' in EPrime_Info['exp_in_file']):
